  ...
  server.registerTool(
    'obtener_puntos_interes',
    {
      ...
      inputSchema: {
        direccion: z
          .string()
          .describe('Dirección completa de la propiedad (ej: Calle Alonso Cano 25, Madrid)'),
        tipos: z
        ...
        radio_metros: z
          .number()
          .default(1000)
          .describe('Radio de búsqueda en metros (por defecto 1000)'),
      },
    },
    async ({ direccion, tipos, radio_metros }) => {
      ...
      if (puntos.length === 0) {
        return {
          content: [
            { type: 'text', text: `No se encontraron puntos de interés cerca de: ${direccion}` },
          ],
        };
      }
