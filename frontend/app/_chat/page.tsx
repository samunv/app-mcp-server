...
export default function Chat() {
  ...
  return (
    <div>
      {messages.map((msg, i) => (
        <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} p-4`}>
            ...
            {msg.role === "user" ? (
              ...
              <ReactMarkdown
                ...
                components={{
                  ...
                  th: ({ children }) => <th className="px-4 py-2 text-left font-semibold border border-white/20">{children}</th>,
                  td: ({ children }) => <td className="px-4 py-2 border border-white/10 text-gray-300">{children}</td>,
